        ...
        public async Task<string> ExportRentDepositLedgerAsync(LedgerExportModel model, CancellationToken ct)
        {
            ...
            return await Task.Run(() =>
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    // Match the visual style from the reference sheet:
                    // - Rent / Deposit columns in soft green
                    // - Collected by columns with a light peach tone
                    ...
                    ws.Cell(currentRow, 3).Style.Fill.SetBackgroundColor(XLColor.FromArgb(255, 255, 230, 204));
                    ws.Cell(currentRow, 5).Style.Fill.SetBackgroundColor(XLColor.FromArgb(255, 255, 230, 204));
                ...
                {
                    // Center everything in the main table, except keep Names left-aligned
                    var bodyRange = ws.Range(dataStartRow, 1, dataEndRow, 5);
                    bodyRange.Style.Alignment.SetVertical(XLAlignmentVerticalValues.Center);
                    bodyRange.Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);
                    ws.Range(dataStartRow, 1, dataEndRow, 1).Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Left);
                }
                ...
                ws.Range(currentRow, 1, currentRow, 2).Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);
                currentRow++;
                ...
                ws.Cell(currentRow, 2).Style.Fill.SetBackgroundColor(XLColor.FromArgb(255, 255, 230, 204));
                ws.Cell(currentRow, 2).Style.NumberFormat.Format = "£ #,##0.00";
                ws.Range(currentRow, 1, currentRow, 2).Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);
                currentRow++;
