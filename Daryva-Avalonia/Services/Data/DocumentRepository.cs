...
namespace Daryva.Services.Data
{
    ...
    public class DocumentRepository : IDocumentRepository
    {
        ...
        public async Task DeleteDocumentsByTenantIdAsync(int tenantId)
        {
            var sql = "DELETE FROM Document WHERE TenantId = @TenantId";
            await Task.FromResult(_dbContext.ExecuteNonQuery(sql, new { TenantId = tenantId }));
        }

        public async Task DeleteDocumentsByTenancyIdAsync(int tenancyId)
        {
            var sql = "DELETE FROM Document WHERE TenancyId = @TenancyId";
